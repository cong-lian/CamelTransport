...
public class CamelDpTest
{
    [Fact(DisplayName = "non-positive capacity")]
    public void InvalidCapacity()
    {
        Assert.Throws<ArgumentException>(() => new CamelDp(-10));
        Assert.Throws<ArgumentException>(() => new CamelDp(0));

        Assert.Null(Record.Exception(() => new CamelDp(1)));
        Assert.Null(Record.Exception(() => new CamelDp(2)));
        Assert.Null(Record.Exception(() => new CamelDp(1000)));
        Assert.Null(Record.Exception(() => new CamelDp(1000000000)));
    }

    [Fact(DisplayName = "non-positive number of bananas")]
    public void InvalidNumberOfBananas()
    {
        var camel = new CamelDp(1000);
    ...
    public void InvalidMiles()
    {
        ...
        Assert.Null(Record.Exception(() => camel.Transport(1, 1000)));
    }
